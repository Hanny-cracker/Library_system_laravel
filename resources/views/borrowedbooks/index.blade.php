    ...
    <div class="bg-white py-20 sm:py-30">
        <div class="mx-auto max-w-7xl px-3 lg:px-8">
            <div class="pb-5 text-center mx-auto max-w-2xl lg:mx-0 border-b border-gray-200">
                <h2 class="text-4xl font-semibold tracking-widest text-pretty text-gray-900 sm:text-5xl">Checked-Out Books</h2>
                <div class="flex justify-center gap-3">
                    <button>
                        <a href="{{route('books')}}"
                            class="mt-6 inline-block bg-black px-4 py-2 text-sm/6 font-semibold text-white hover:bg-pretty-dark rounded-lg">Check-Out Book</a>
                    </button>
                    
                </div>

                <form class="flex justify-center mt-6 max-w-md mx-auto" action="{{ route('borrowedbook.search') }}" method="GET">
                    <div class="flex justify-center w-full max-w-sm min-w-[200px]">
                        <div class="relative">
                            <input
                                ...
                                placeholder="Search by Name, Email..." name="search" id="search"
                                value="{{  $search ?? '' }}" />
                            @if(!$search)
                            <button
                            ...
                            <button
                                class="absolute top-1 right-1 flex items-center rounded bg-slate-800 py-1 px-2.5 border border-transparent text-center text-sm text-white transition-all shadow-sm hover:shadow focus:bg-slate-200 focus:shadow-none active:bg-slate-700 hover:bg-slate-500 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                                <a href="{{ route('borrowedbooks') }}">Clear</a>
                            </button>
                            @endif
                        </div>
                    </div>
            ...
            <div class="mx-auto mt-5 grid max-w-2xl grid-cols-4 gap-x-12 gap-y-12 sm:mt-16 sm:pt-1 lg:mx-0 lg:max-w-none lg:grid-cols-4">
            ...
            @else
            <div class="mx-auto mt-5 grid max-w-2xl grid-cols-1 gap-x-12 gap-y-12 sm:mt-16 sm:pt-1 lg:mx-0 lg:max-w-none lg:grid-cols-4">
                @foreach($borrowedbooks as $borrowedbook)
                <x-view-borrowedbook :borrowedbook="$borrowedbook" />
                @endforeach
            </div>
            @endif
            <!-- More posts... -->
